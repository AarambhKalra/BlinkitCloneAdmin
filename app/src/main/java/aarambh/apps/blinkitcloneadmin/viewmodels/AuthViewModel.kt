...
import aarambh.apps.blinkitcloneadmin.models.Admin
...
class AuthViewModel: ViewModel() {
    ...
    fun sendOTP(adminNumber: String, activity: Activity) {
        ...
        val options = PhoneAuthOptions.newBuilder(Utils.getAuthInstance())
            .setPhoneNumber("+91$adminNumber") // Phone number to verify
    ...
    fun signInWithPhoneAuthCredential(otp: String, adminNumber: String) {
        ...
        Utils.getAuthInstance().signInWithCredential(credential).addOnCompleteListener { task ->
            if (task.isSuccessful) {
                ...
                if (firebaseUser != null) {
                    val user = Admin(uid = firebaseUser.uid, adminPhoneNumber = adminNumber)
